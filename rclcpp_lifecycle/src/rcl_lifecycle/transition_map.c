...
void rcl_register_primary_state(rcl_transition_map_t * m,
...
void rcl_register_transition_by_state(rcl_transition_map_t * m,
...
void rcl_register_transition_by_label(rcl_transition_map_t * m,
  const char * start_label, const char * goal_label, rcl_state_transition_t transition)
{
  ...
  // TODO(karsten1987): Not sure if this is really necessary to differentiate
...
void rcl_register_transition_by_index(rcl_transition_map_t * m,
...
rcl_state_t * rcl_get_primary_state_by_label(rcl_transition_map_t * m,
  const char * label)
{
...
rcl_state_t * rcl_get_primary_state_by_index(rcl_transition_map_t * m,
  unsigned int index)
{
  for (unsigned int i = 0; i < m->size; ++i) {
...
rcl_transition_array_t * rcl_get_transitions_by_label(rcl_transition_map_t * m,
  const char * label)
{
  for (unsigned int i = 0; i < m->size; ++i) {
...
rcl_transition_array_t * rcl_get_transitions_by_index(rcl_transition_map_t * m,
  unsigned int index)
{
  for (unsigned int i = 0; i < m->size; ++i) {
    if (m->primary_states[i].index == index) {
